...
### 12.2. 함수적 종속
- 어떤 테이블 R에서 X와 Y가 각각 R의 속성 집합의 부분 집합이라 하고, 속성 X의 값 각각에 대해 시간에 관계 없이 항상 속성 Y의 값이 오직 하나만 연견되어 있을 때 Y는 X에 함수적 종속 또는 X가 시킬 함수적으로 결정한다고 하고, X -> Y로 표기한다.
...
### 22.1. DAS (Direct Attached Storage)
...
### 22.2. NAS (Network Attahced Storage)
